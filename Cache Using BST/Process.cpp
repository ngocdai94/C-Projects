...
bool readFiles(Cache& cacheName, double* result,
			   string fileName,  string commandName);
...
int main()
{
  ...
  while (getline(infile, readData)){
	...
	// Max
	if(commandName == "Max"){	
	  if (simpleCache.checkMax(fileName) == NOT_FOUND_MAX_MIN){
		// else read data file to find max
		readFiles(simpleCache, &result, fileName, commandName);
	  } else {
		// retrieve data from cache
		// cast integer return type to double
		result=static_cast<int>(simpleCache.checkMax(fileName));
	  }
	...
	// Min
	if (commandName == "Min"){
	  if (simpleCache.checkMin(fileName) == NOT_FOUND_MAX_MIN){
		// else read data file to find min
		readFiles(simpleCache, &result, fileName,  commandName);
	  } else {
		// retrieve data from cache
		// cast integer return type to double   
		result=static_cast<int>(simpleCache.checkMin(fileName));
	  }
	  // print out result to the prompt and result.out file
	  elapsed_secs = double(clock() - begin) / CLOCKS_PER_SEC;
	...
	// Avg
	if (commandName == "Avg"){
	  if (simpleCache.checkAvg(fileName) == NOT_FOUND_AVG_MED){
		// else read data file to find Avg
		readFiles(simpleCache, &result, fileName, commandName);
	  } else {
		// retrieve data from cache
		// cast integer return type to double   
		result = simpleCache.checkAvg(fileName);
	  }
	  // print out result to the prompt and result.out file
	  elapsed_secs = double(clock() - begin) / CLOCKS_PER_SEC;
	...
	// Med
	if (commandName == "Med"){
	  if (simpleCache.checkMed(fileName) == NOT_FOUND_AVG_MED){
		// else read data file to find Med
		readFiles(simpleCache, &result, fileName, commandName);
	  } else {
		// retrieve data from cache
		// cast integer return type to double
		result = simpleCache.checkMed(fileName);
	  }
	  // print out result to the prompt and result.out file
	  elapsed_secs = double(clock() - begin) / CLOCKS_PER_SEC;
  ...
  
{
  ...
  // save max to cache
  if (commandName == "Max"){
	cacheName.saveMax(fileName, max);
  ...
  // save min to cache
  if (commandName == "Min"){
	cacheName.saveMin(fileName, min);
  ...
  if (commandName == "Avg"){
	avg = total/count;
	cacheName.saveAvg(fileName, avg);
	*result = avg;
  ...
  if (commandName == "Med") {
	cacheName.saveMed(fileName, tempArr, med, count);
	*result = med;
  }
